...
async function getTokensFor(
  prefField: keyof Pick<UserDoc, 'notifPosts' | 'notifAnnounce' | 'notifStories' | 'notifResources' | 'notifEvents' | 'notifTeam' | 'notifBdays'>,
  ...
  for (const doc of snap.docs) {
    ...
    if (user[prefField] === false) continue; // explicitly opted out
...
export const onPostCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const authorFirst = (post.author as string).split(' ')[0];
    const title = isAnnouncement ? '📣 New Announcement' : `💬 ${authorFirst} posted`;
    const body = (post.text as string).slice(0, 120) || 'New post in Community';
...
export const onStoryCreated = onDocumentCreated(
  { ...DB_OPT, document: 'stories/{storyId}' },
  async (event) => {
...
export const onDocumentUploaded = onDocumentCreated(
  { ...DB_OPT, document: 'documents/{docId}' },
  async (event) => {
...
/** New user added */
export const onUserAdded = onDocumentCreated(
  { ...DB_OPT, document: 'users/{userId}' },
  async (event) => {
...
export const onEventApproved = onDocumentUpdated(
  { ...DB_OPT, document: 'events/{eventId}' },
  async (event) => {
...
export const dailyBirthdayCheck = onSchedule(
  { schedule: '0 13 * * *', timeZone: 'America/Chicago' }, // 8 AM CST = 13:00 UTC
  async () => {
    ...
    for (const person of birthdayPeople) {
      await sendPush(
        notifTokens.filter(t => !(person.fcmTokens ?? []).includes(t)), // don't notify yourself
        '🎂 Birthday Today!',
        `Today is ${person.name}'s birthday — send them a message!`,
        { tab: 'home' },
      );
    }
    ...
    for (const person of anniversaryPeople) {
      ...
      await sendPush(
        notifTokens.filter(t => !(person.fcmTokens ?? []).includes(t)),
        '🎉 Work Anniversary!',
        `${person.name} is celebrating ${years} year${years === 1 ? '' : 's'} at Stewpot today!`,
        { tab: 'home' },
      );
    }
  },
